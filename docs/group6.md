...
Once finished, you will be taken to the `get_remaining_runway` function page. There's a lot of menu items on th is page - don't be alarmed! We're going to come back to this page in a moment.
...
For this guide, we're going to use the contents of the `get_remaining_runway.py` file found in the repository on GitHub. Open the [`get_remaining_runway.py` file](/intents/get_remaining_runway.py) and copy the code.

Go back to AWS and locate the Function Code window and delete any code that already exists in the lambda_function window. Then, paste the contents of the `get_remaining_runway.py` file into the Function Code window. The resulting output should look like the image below:

![Function Code for get_remaining_runway](/docs/images/get-cluster-status-function-code.jpg)
...
![Environmental Variables for get_remaining_runway](/docs/images/environmental-variables.jpg)
...
![Test Content for get_remaining_runway](/docs/images/get-cluster-status-test-content.jpg)
...
1.  Finally, give this intent a name. We're going to use the name `get_remaining_runway`.
...
We'll pick several phrases from the `get_remaining_runway.py` example and enter them below:

1.  How much runway is left
1.  How much runway do I have left
1.  What is the remaining runway on my cluster
...
Navigate to the fulfillment section of the intent, choose the **AWS Lambda function** radio button, and then use the drop down menu to pick the `get_remaining_runway` function with the version default of Latest.
...
*   Intent-Name: This should match the intent based on the sample utterances provided int the intent. In this case, you should see the value of `get_remaining_runway`.
*   Dialog-State: Fulfilled
*   Input-Transcript: The words you spoke into the microphone.
*   Message: The response from Roxie, as generated by the Lambda function `get_remaining_runway`.
