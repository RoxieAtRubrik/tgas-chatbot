...
Once finished, you will be taken to the `open_support_tunnel` function page. There's a lot of menu items on th is page - don't be alarmed! We're going to come back to this page in a moment.
...
For this guide, we're going to use the contents of the `open_support_tunnel.py` file found in the repository on GitHub. Open the [`open_support_tunnel.py` file](/intents/open_support_tunnel.py) and copy the code.

Go back to AWS and locate the Function Code window and delete any code that already exists in the lambda_function window. Then, paste the contents of the `open_support_tunnel.py` file into the Function Code window. The resulting output should look like the image below:

![Function Code for open_support_tunnel](/docs/images/get-cluster-status-function-code.jpg)
...
![Environmental Variables for open_support_tunnel](/docs/images/environmental-variables.jpg)
...
![Test Content for open_support_tunnel](/docs/images/get-cluster-status-test-content.jpg)
...
1.  Finally, give this intent a name. We're going to use the name `open_support_tunnel`.
...
We'll pick several phrases from the `open_support_tunnel.py` example and enter them below:

1.  Create support tunnel
1.  Open support tunnel
...
Navigate to the fulfillment section of the intent, choose the **AWS Lambda function** radio button, and then use the drop down menu to pick the `open_support_tunnel` function with the version default of Latest.
...
*   Intent-Name: This should match the intent based on the sample utterances provided int the intent. In this case, you should see the value of `open_support_tunnel`.
*   Dialog-State: Fulfilled
*   Input-Transcript: The words you spoke into the microphone.
*   Message: The response from Roxie, as generated by the Lambda function `open_support_tunnel`.
