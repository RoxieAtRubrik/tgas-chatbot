...
For this guide, we're going to use the contents of the `get_node_count.py` file found in the repository on GitHub. Open the [`get_node_count.py` file](/intents/get_node_count.py) and copy the code.

Go back to AWS and locate the Function Code window and delete any code that already exists in the lambda_function window. Then, paste the contents of the `get_node_count.py` file into the Function Code window. The resulting output should look like the image below:

![Function Code for get_node_count](/docs/images/get-cluster-status-function-code.jpg)
...
![Test Content for get_node_count](/docs/images/get-cluster-status-test-content.jpg)
...
1.  Finally, give this intent a name. We're going to use the name `get_node_count`.
...
We'll pick several phrases from the `get_node_count.py` example and enter them below:

1.  What is the node count
1.  How many nodes are in the cluster
1.  What is the number of nodes in the cluster
...
Navigate to the fulfillment section of the intent, choose the **AWS Lambda function** radio button, and then use the drop down menu to pick the `get_node_count` function with the version default of Latest.
...
*   Intent-Name: This should match the intent based on the sample utterances provided int the intent. In this case, you should see the value of `get_node_count`.
*   Dialog-State: Fulfilled
*   Input-Transcript: The words you spoke into the microphone.
*   Message: The response from Roxie, as generated by the Lambda function `get_node_count`.
