...
For this step, it's important to add three environment variables: `CLUSTER_IP`, `USERNAME`, and `PASSWORD`. This can be done by scrolling down below the Function Code area and locating the Environmental Variables area. This information is provided in the image below.
...
1.  Finally, give this intent a name. We're going to use the name `get_archived_amount`.
...
1.  How much data has been archived
1.  Give me details of archival storage
1.  What is the amount of archived data
...
*   Intent-Name: This should match the intent based on the sample utterances provided int the intent. In this case, you should see the value of `get_archived_amount`.
